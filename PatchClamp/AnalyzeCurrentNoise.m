...
StartOffBelow = []; EndeRamp = [];LengthInms =[];ActuSensorAvg=[];StartOffBelowShort=[];

%%% this could go to the ForceClamp Section  
ActuSensorAvg = tsmovavg(ActuSensor,'s',10,1);%ToDO: error!!!
%  
for i = 1:size(Aall,2);
...
StartOffBelow(i) = StartOffBelowShort(i)+ CellMaxActuFirst(i); % CellMaxActuFirst excludes the values before Max Value
    ...
    if isFiveStep == 1 || isStep ==1 || isFifteenStep ==1 || isFivePosStep == 1|| isSixteenStep == 1
